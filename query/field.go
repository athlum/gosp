...
func (f *Field) String() string {
	full := f.full
	if f.method != nil {
		full = fmt.Sprintf("%v(%v)", *f.method, full)
	}
	if f.as != nil {
...
func (f *Field) Method(m string) *Field {
	nf := f.Fork()
	nf.method = &m
	return nf
}
...
func (f *Field) INSubQuery(value string) *Condition {
	return f.INStr(f.subQueryString(value))
}

func (f *Field) NOTINSubQuery(value string) *Condition {
	return f.NOTINStr(f.subQueryString(value))
}
